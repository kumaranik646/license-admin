    ...
    if request.method == 'POST':
        try:
            ...
            if expiry_type == 'custom':
                expiry_date_str = request.form.get('expiry_date_custom')
                if expiry_date_str:
                    license.expiry_date = datetime.strptime(expiry_date_str, '%Y-%m-%d')
            elif expiry_type == 'datetime':
                expiry_datetime_str = request.form.get('expiry_datetime_custom')
                if expiry_datetime_str:
                    license.expiry_date = datetime.strptime(expiry_datetime_str, '%Y-%m-%dT%H:%M')
            elif expiry_type == 'extend':
                extend_value = int(request.form.get('extend_value', 0))
                extend_unit = request.form.get('extend_unit', 'days')
                if extend_unit == 'days':
                    license.expiry_date = license.expiry_date + timedelta(days=extend_value)
                elif extend_unit == 'months':
                    license.expiry_date = license.expiry_date + timedelta(days=extend_value * 30)
                elif extend_unit == 'years':
                    license.expiry_date = license.expiry_date + timedelta(days=extend_value * 365)
            else:
                expiry_value = int(request.form.get('expiry_value', 365))
                expiry_unit = request.form.get('expiry_unit', 'days')
                if expiry_unit == 'days':
                    license.expiry_date = get_current_time() + timedelta(days=expiry_value)
                elif expiry_unit == 'hours':
                    license.expiry_date = get_current_time() + timedelta(hours=expiry_value)
                elif expiry_unit == 'minutes':
                    license.expiry_date = get_current_time() + timedelta(minutes=expiry_value)
